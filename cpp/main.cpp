// clang++ -std=c++20 $(pkg-config --cflags --libs  arrow-csv) main.cpp &&
// ./a.out && rm a.out

// cd ../src && python setup.py build && python setup.py install && cd - &&
// python test_all.py
...
double q6() {
  ...
  // TODO (probably never)
  // assumption: functional map/reduce simplifies translation from SDQL to C++
  // observation: below shows functional map/reduce sucks in C++ 23
  // idea: Rust is compiled and has a good functional map/reduce – use it?
  //
  // auto scalar = 0.0;
  // for (const auto &[a, b, c, d]:
  //      std::views::zip(l_shipdate, l_discount, l_quantity, l_extendedprice))
  //      {
  //   if (a >= "1994-01-01" && a < "1995-01-01" && b >= 0.05 && b <= 0.07 &&
  //       c < 24.0) {
  //     scalar += d * b;
  //   }
  // }
  //
  // // double view =
  // //         std::views::zip(l_shipdate, l_discount, l_quantity,
  // l_extendedprice) |
  // //         std::views::filter([](auto tup) {
  // //           return std::get<0>(tup) >= "1994-01-01" &&
  // //                  std::get<0>(tup) < "1995-01-01" && std::get<1>(tup) >=
  // 0.05 &&
  // //                  std::get<1>(tup) <= 0.07 && std::get<2>(tup) < 24.0;
  // //         })
  // //     // | std::transform(view, [](auto tup) { return get<3>(tup) *
  // get<1>(tup); })
  // //     ;
  //
  // // auto res = std::reduce(view.begin(), view.end(), 0, [](auto a, auto b) {
  // //   return (a + b);
  // // });
  //
  // auto vec = {1, 2, 3};
  // // auto res = std::reduce(vec.begin(), vec.end(), 0, [](int a, int b) {
  // // return a + b;
  // // });
  // auto res = std::reduce(vec.begin(), vec.end(), 0, std::plus<int>());
...
//     arrow::csv::ReadOptions({.autogenerate_column_names = true}); const auto
//     parse_options = arrow::csv::ParseOptions({.delimiter = '|'}); const auto
...
//     in(DATASETS_DIR / "lineitem.tbl"); in.set_header(
...
//     std::string _14;
//     std::string _15;
//     while (in.read_row(_0, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12,
//     _13, _14, _15) ) {
//         std::cout << _0 << "|" << _1 << "|" << _2 << "|" << _3 << "|" << _4
//         << "|" << _5 << "|" << _6 << "|" << _7 <<
//                 "|" << _8 << "|" << _9 << "|" << _10 << "|" << _11 << "|" <<
//                 _12 << "|" << _13 << "|" << _14 << "|" << _15 << std::endl;
//     }
// }
...
//     };
//     const auto format =
//     csv::CSVFormat().delimiter('|').column_names(colnames); const auto
//     filename = DATASETS_DIR / "lineitem.tbl";
//
//     for (csv::CSVReader reader(filename.generic_string(), format);
//     csv::CSVRow &row: reader) {
