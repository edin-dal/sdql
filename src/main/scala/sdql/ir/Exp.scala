...
sealed trait LLQL
case class Initialise(tpe: Type, agg: Aggregation)                                        extends Exp with LLQL // TODO infer aggregation from type?
case class Update(accessors: Seq[Exp], inner: Exp, agg: Aggregation, destination: String) extends Exp with LLQL
case class Modify(accessors: Seq[Exp], inner: Exp, destination: String)                   extends Exp with LLQL
