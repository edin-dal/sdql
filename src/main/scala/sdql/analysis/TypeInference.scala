...
object TypeInference {
	...
	def run(e: Exp)(implicit ctx: Ctx): Type = {
		e match {
			...
			case RecNode(values) =>
				RecordType(values.map(_._1).zip(values.map(_._2).map(run)).map(x => Attribute(x._1, x._2)))
			...
			case Load(_, tp) =>
				tp match {
					case DictType(tp1, _) => RelationType(tp1)
	...
	def sum_with_ctx(e: Sum)(implicit ctx: Ctx): (Type, Ctx) = {
		...
		val (kType, vType) = ctx.get(e1Sym) match {
			...
			case Some(RelationType(k_type)) => (k_type, IntType)
			case Some(tpe) => raise(
